...
│       ├── eval/*.json               # Docker 테스트 검증 결과 (Step 2)
│       ├── reports/                  # 리포트 (Step 3, Markdown/JSON/CSV)
│       ├── metadata.json             # 실행 메타데이터
│       ├── summary.json              # 대시보드용 요약 (지표/등급)
│       └── run.log                   # 실행 로그
│
├── pyproject.toml
...
(run_eval.py)               (run_docker_eval.py)         (generate_report.py)
 ...
 base_commit checkout              │                          │
      │                      agent patch 적용             비교 리포트 생성
 에이전트 실행                      │                    (Markdown/JSON/CSV)
 (claude -p ...)             테스트 파일 경로 초기화
      │                      (test_patch 충돌 방지)
 git diff → 패치 추출               │
      │                      test_patch 적용
 결과 저장                          │
...
**그래서 Step 2가 필요합니다.** SWE-bench는 인스턴스별로 정확한 개발환경이 세팅된 Docker 이미지를 제공합니다. 이 이미지 안에는 해당 프로젝트의 정확한 Python 버전, 모든 의존성, conda 환경이 `/testbed` 디렉토리에 사전 설치되어 있습니다. Step 2의 실행 순서는 다음과 같습니다:

1. **agent patch 적용** — Step 1에서 저장한 patch를 `git apply`로 `/testbed`에 적용합니다. strict → `--3way` → `--3way --ignore-whitespace` 3단계 폴백을 시도하며, 모두 실패하면 `fail`로 기록합니다.
2. **테스트 파일 경로 초기화** — 에이전트가 테스트 파일까지 수정한 경우를 대비해, test_patch가 건드릴 경로를 `git checkout HEAD --`로 베이스 상태로 되돌립니다. SWE-bench 공식 하네스와 동일한 처리 방식으로, 에이전트는 프로덕션 코드만 평가받습니다.
3. **test_patch 적용** — SWE-bench가 제공하는 검증용 테스트 패치를 적용합니다. 이 패치가 FAIL_TO_PASS 테스트(버그 재현 테스트)를 포함하고 있습니다.
4. **언어별 후처리 훅** — C++ 등 컴파일이 필요한 언어는 이 시점에 재빌드합니다. Python은 no-op입니다.
5. **FAIL_TO_PASS 테스트 실행** → **PASS_TO_PASS 테스트 실행**
