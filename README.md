...
에이전트 어댑터(`src/adapters/claude_code.py`)는 Anthropic API를 직접 호출하지 않습니다. Claude Code CLI 바이너리를 `subprocess.Popen`으로 실행합니다:
...
- Python 3.10+
- Docker (테스트 검증용)
- Claude Code CLI (`claude` 명령어) 또는 OpenCode CLI (`opencode`)
